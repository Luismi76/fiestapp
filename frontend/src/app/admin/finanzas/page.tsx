...
import PendingTasksCard, { PendingTask } from '@/components/admin/PendingTasksCard';
...
const DEFAULT_KPIS: DashboardKpi = {
  ...
  packPurchases: 0, packPurchasesChange: 0,
  experiencePayments: 0, experiencePaymentsChange: 0,
  refunds: 0, refundsChange: 0,
  totalWalletBalance: 0,
  depositPaidCount: 0, depositPaidAmount: 0,
  balancePendingCount: 0, balancePendingAmount: 0,
  operationsCount: 0, operationsCountChange: 0,
};
...
function ResumenTab() {
  ...
  const fetchDashboard = useCallback(async () => {
    ...
    setError('');
    try {
      const [data, walletData, tasksData] = await Promise.all([
        ...
        api.get('/admin/tasks/pending').then((r) => r.data as PendingTask[]).catch(() => [] as PendingTask[]),
      ]);
  ...
  return (
    <div className="space-y-4">
      {/* Tareas pendientes (solo fiscal/operativa crítica) */}
      ...
      {/* Packs vendidos y saldos en monedero */}
      <div className="space-y-2">
        <div className="flex items-center gap-2 px-1">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 md:gap-4">
          <KpiCard
            ...
            change={kpis.packPurchasesChange}
            icon={
      ...
      {/* Reservas con depósito (payment plans) */}
      <div className="space-y-2">
        <div className="flex items-center gap-2 px-1">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 md:gap-4">
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-xs text-purple-600 font-semibold">{kpis.depositPaidCount}</span>
            </div>
            <div className="text-2xl font-bold text-gray-900">{formatEur(kpis.depositPaidAmount)}</div>
            <div className="text-[11px] text-gray-400 mt-1">Importes recibidos como depósito</div>
          </div>
          <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-4">
            <div className="flex items-center justify-between mb-2">
              <span className="text-xs font-medium text-gray-500">SALDOS PENDIENTES DE COBRO</span>
              <span className="text-xs text-amber-600 font-semibold">{kpis.balancePendingCount}</span>
            </div>
            <div className="text-2xl font-bold text-gray-900">{formatEur(kpis.balancePendingAmount)}</div>
            <div className="text-[11px] text-gray-400 mt-1">A cobrar automáticamente antes de la experiencia</div>
          </div>
        </div>
      </div>
