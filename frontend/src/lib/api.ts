...
export interface Evaluation {
  ...
  page: string;
  category: 'PROBLEMA' | 'MEJORA' | 'OPINION';
...
// Cliente sin autenticación JWT — usa header x-eval-code
...
export const evaluationsApi = {
  verifyCode: async (code: string): Promise<{ valid: boolean }> => {
    const response = await evalApi.post<{ valid: boolean }>('/evaluations/verify-code', { code });
    return response.data;
  },

  create: async (data: CreateEvaluationData, evalCode: string): Promise<Evaluation> => {
    const response = await evalApi.post<Evaluation>('/evaluations', data, {
      headers: { 'x-eval-code': evalCode },
    });
    return response.data;
  },

  // Endpoints admin (usan JWT)
  getAll: async (filters?: {
    ...
    const params = buildQueryParams(filters as Record<string, unknown>);
    const response = await api.get<EvaluationsResponse>(`/evaluations?${params}`);
    return response.data;
  },
  ...
  update: async (id: string, data: { status?: string; adminNotes?: string }): Promise<Evaluation> => {
