...
export class CategoriesService implements OnModuleInit {
  ...
  async seed() {
    const categories = [
      // Fiestas populares
      { name: 'Feria', slug: 'feria', group: 'fiesta', icon: '🎪', sortOrder: 1 },
      { name: 'Romería', slug: 'romeria', group: 'fiesta', icon: '⛪', sortOrder: 2 },
      { name: 'Procesión', slug: 'procesion', group: 'fiesta', icon: '🕯️', sortOrder: 3 },
      { name: 'Carnaval', slug: 'carnaval', group: 'fiesta', icon: '🎭', sortOrder: 4 },
      { name: 'Verbena / Fiesta patronal', slug: 'verbena', group: 'fiesta', icon: '🎆', sortOrder: 5 },
      { name: 'Encierro / Fiesta taurina', slug: 'encierro', group: 'fiesta', icon: '🐂', sortOrder: 6 },
      { name: 'Festival / Concierto', slug: 'festival', group: 'fiesta', icon: '🎵', sortOrder: 7 },
  ...
  async create(dto: CreateCategoryDto) {
    ...
    if (existing) {
      throw new ConflictException(`Ya existe una categoría con slug "${dto.slug}"`);
    }
