...
export class AuthController {
  ...
  @Post('register')
  @ApiOperation({ summary: 'Registrar nuevo usuario', description: 'Crea una nueva cuenta de usuario y envía email de verificación' })
  @ApiBody({ type: RegisterDto })
  @ApiResponse({ status: 201, description: 'Usuario registrado correctamente. Se ha enviado email de verificación.' })
  ...
  @ApiResponse({ status: 200, description: 'Login exitoso. Devuelve access_token y datos del usuario.' })
  ...
  @Get('verify-email')
  @ApiOperation({ summary: 'Verificar email', description: 'Verifica la dirección de email usando el token enviado por correo' })
  @ApiQuery({ name: 'token', description: 'Token de verificación recibido por email' })
  ...
  @ApiOperation({ summary: 'Reenviar email de verificación', description: 'Envía un nuevo email de verificación al usuario' })
  @ApiBody({ type: ResendVerificationDto })
  @ApiResponse({ status: 200, description: 'Email de verificación reenviado (si existe)' })
  @ApiResponse({ status: 400, description: 'Email ya verificado o espera requerida' })
  ...
  @ApiOperation({ summary: 'Solicitar restablecimiento de contraseña', description: 'Envía un email con link para restablecer contraseña' })
  @ApiBody({ type: ForgotPasswordDto })
  @ApiResponse({ status: 200, description: 'Email enviado si la cuenta existe' })
  @ApiResponse({ status: 429, description: 'Rate limit excedido' })
  @Throttle({ short: { limit: 1, ttl: 1000 }, medium: { limit: 3, ttl: 60000 }, long: { limit: 10, ttl: 3600000 } })
  ...
  @Post('reset-password')
  @ApiOperation({ summary: 'Restablecer contraseña', description: 'Establece una nueva contraseña usando el token de restablecimiento' })
  @ApiBody({ type: ResetPasswordDto })
  @ApiResponse({ status: 200, description: 'Contraseña actualizada correctamente' })
  @ApiResponse({ status: 400, description: 'Token inválido o expirado' })
  @ApiResponse({ status: 429, description: 'Rate limit excedido' })
  @Throttle({ short: { limit: 2, ttl: 1000 }, medium: { limit: 5, ttl: 60000 }, long: { limit: 15, ttl: 3600000 } })
  ...
  @Get('me')
  @ApiOperation({ summary: 'Obtener perfil actual', description: 'Devuelve los datos del usuario autenticado' })
  @ApiBearerAuth('JWT')
  ...
  @Post('2fa/login')
  @Throttle({ short: { limit: 3, ttl: 1000 }, medium: { limit: 5, ttl: 60000 }, long: { limit: 15, ttl: 3600000 } })
  async verifyTwoFactorLogin(@Body() dto: LoginWith2FADto) {
    return this.authService.verifyTwoFactorLogin(dto.tempToken, dto.twoFactorCode);
  }
